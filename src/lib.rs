...
pub fn encrypt(
    ...
    let mut tag: [u8; 16] = rng.gen();
    output.append(
        &mut c
            .encrypt(&Nonce::from_slice(&tag[..12]), message)
            .map_err(|_| EncryptError::EncryptionFailure)?,
    );
...
pub fn decrypt(secret_key: &[u8; 32], message: &[u8]) -> Result<Vec<u8>, DecryptError> {
    ...
    // Ok(plaintext)
    return decrypter
        .decrypt(&Nonce::from_slice(&tag[..12]), ciphertext)
        .map_err(|e| DecryptError::Invalid);
}
